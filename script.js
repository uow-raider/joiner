...
async function joinServer(token, inviteCode, num, captchaData = null) {
    ...
    try {
        ...
        if (res.ok && data.code) {
            ...
            addLog('JOIN: Captcha Required', 'warn', num);
            ...
            addLog(`JOIN ERROR: ${res.status} - ${data.message || 'Check logs'}`, 'error', num);
            processNextCaptcha();
        }
    ...
    if (msgUrl && msgUrl.includes('channels/')) {
        try {
            ...
            if (msgRes.ok) {
                ...
                addLog('BYPASS: Sequence Done', 'success', num);
            }
        } catch(e) { addLog('BYPASS: Error', 'error', num); }
    }
}

// --- Update Display Name Engine ---
async function updateProfile(token, num) {
    ...
    if (avatarUrl) {
        ...
        } catch (e) { addLog('AVATAR: Fetch Error', 'error', num); }
    }
    
    if (Object.keys(payload).length > 0) {
        const res = await fetch(`https://discord.com/api/v9/users/@me`, { method: 'PATCH', headers, body: JSON.stringify(payload) });
        if (res.ok) {
            addLog(`DISPLAY PROFILE: Updated to "${displayName || 'Avatar'}"`, 'success', num);
        } else {
            const err = await res.json();
            addLog(`DISPLAY PROFILE: ${res.status} - ${err.message || 'Rate Limited'}`, 'error', num);
        }
    }
}
